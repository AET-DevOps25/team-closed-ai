...
public class TaskControllerTest {
    ...
    mockMvc
        .perform(
            put("/tasks/1")
                ...
                .content(
                    "{\"id\":1,\"title\":\"Updated Task\",\"description\":\"Updated Description\",\"taskStatus\":\"IN_PROGRESS\",\"comments\":[\"Comment 1\"],\"attachments\":[\"Attachment 1\"],\"assigneeId\":2}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id", is(1)))
    ...
    mockMvc
        .perform(
            put("/tasks/99")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"id\":99,\"title\":\"Updated Task\",\"description\":\"Updated Description\",\"taskStatus\":\"IN_PROGRESS\",\"comments\":[\"Comment 1\"],\"attachments\":[\"Attachment 1\"],\"assigneeId\":1}"))
  ...
  void deleteTask_WhenTaskDoesNotExist_ShouldReturnNotFound() throws Exception {
    ...
    // Perform DELETE request and validate response
    mockMvc.perform(delete("/tasks/99")).andExpect(status().isNotFound());
  ...
  void changeStatus_ShouldUpdateTaskStatus() throws Exception {
    ...
    mockMvc
        .perform(patch("/tasks/1/status").param("status", "DONE"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id", is(1)))
        .andExpect(jsonPath("$.title", is("Test Task 1")))
        .andExpect(jsonPath("$.description", is("Description 1")))
  ...
  void getTasksByAssignee_WhenUserExists_ShouldReturnTasks() throws Exception {
    ...
    // Perform GET request and validate response
    mockMvc
        .perform(get("/tasks/by-assignee/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(2)))
        .andExpect(jsonPath("$[0].id", is(1)))
        .andExpect(jsonPath("$[0].title", is("Test Task 1")))
  ...
  void getTasksByAssignee_WhenUserDoesNotExist_ShouldReturnNotFound() throws Exception {
    ...
    // Perform GET request and validate response
    mockMvc.perform(get("/tasks/by-assignee/99")).andExpect(status().isNotFound());
  ...
  void getTasksByProject_WhenProjectExists_ShouldReturnTasks() throws Exception {
    // Mock service methods
    when(projectServiceClient.findById(1L))
        .thenReturn(Optional.of(ProjectDto.fromProject(testProject)));
    when(taskService.findByProject(1L)).thenReturn(taskList);

    // Perform GET request and validate response
    mockMvc
        .perform(get("/tasks/by-project/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(2)))
        .andExpect(jsonPath("$[0].id", is(1)))
        .andExpect(jsonPath("$[0].title", is("Test Task 1")))
  ...
  void getTasksByProject_WhenProjectDoesNotExist_ShouldReturnNotFound() throws Exception {
    ...
    // Perform GET request and validate response
    mockMvc.perform(get("/tasks/by-project/99")).andExpect(status().isNotFound());
