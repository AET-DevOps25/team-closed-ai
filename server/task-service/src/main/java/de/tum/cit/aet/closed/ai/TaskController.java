...
public class TaskController {
    ...
    public TaskDto updateTask(@PathVariable Long id, @RequestBody TaskDto taskDto) {
        ...
        if ((currentAssigneeId == null && newAssigneeId != null) ||
            ...
            if (newAssigneeId != null) {
                assignee = userServiceClient.findById(newAssigneeId)
                        .map(userDto -> {
                            User user = new User();
                        ...
                        .orElseThrow(() -> new UserNotFoundException("No User found with ID " + newAssigneeId));
            }
    ...
    public List<TaskDto> byProject(@PathVariable("id") Long id) {
        return projectServiceClient.findById(id)
                .map(p -> taskService.findByProject(p.id()).stream()
                        .map(TaskDto::fromTask)
                        .toList())
